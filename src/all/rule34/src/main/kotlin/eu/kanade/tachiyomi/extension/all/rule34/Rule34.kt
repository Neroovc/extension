package eu.kanade.tachiyomi.extension.all.rule34
...
/**
 * Rule34 — Gelbooru-based booru.
 */
class Rule34 : HttpSource() {
    ...
    override fun popularMangaRequest(page: Int): Request {
        val pid = (page - 1) * 42
        return GET("$baseUrl/index.php?page=post&s=list&pid=$pid", headers)
    }
    ...
    private fun parseMangaDetails(doc: Document): SManga {
        return SManga.create().apply {
            ...
            description = buildString {
                append("ID: $postId\n")
                val rating = doc.selectFirst("li.rating-info")
                if (rating != null) {
                    append("Rating: ${rating.text().replace("Rating:", "").trim().uppercase()}\n")
                }
